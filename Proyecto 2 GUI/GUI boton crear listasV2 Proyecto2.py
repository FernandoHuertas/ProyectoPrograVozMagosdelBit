...
def save_list():
    global PeopleList, Lists, listbox_ListofPeople
    PeopleList = list(listbox_ListofPeople.get(0, tk.END))
    list_name = simpledialog.askstring("Nombre de la lista", "Escriba el nombre de la lista:")
    if list_name is not None:
        Lists.append((list_name, PeopleList))
        print(f"{list_name}: {PeopleList}")
        clear_list()
...
def DEFCreateList_MainPopup():
    ...
    button_SaveList = tk.Button(MainpopupCreatelist, text="Guardar lista", command=save_list)
    ...
    button_CloseWindow.place(x=250, y=220)
